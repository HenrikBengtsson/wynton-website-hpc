...
Do you get obscure errors like `\r: command not found` when trying to run your scripts on {{< meta cluster.name >}}?  If so, the most likely explanation is that the problematic script file was created in an editor on MS Windows and then copied to {{< meta cluster.name >}}. If so, there is a simple solution - just run `dos2unix` on the file, e.g.

```sh
[alice@{{< meta devel.name >}} ~]$ dos2unix script.sh
```
...
transfer it to {{< meta cluster.name >}}, and then try to run it there, we get an obscure error:

```sh
[alice@{{< meta devel.name >}} ~]$ source script.sh 
...
```sh
[alice@{{< meta devel.name >}} ~]$ file script.sh
...
```sh
[alice@{{< meta devel.name >}} ~]$ dos2unix --info=d script.sh
       3  script.sh
```

The '3' is the number of CR symbols found in file `script.sh`.  When running on {{< meta cluster.name >}}, we want this count to be zero.
...
```sh
[alice@{{< meta devel.name >}} ~]$ cat -A script.sh
#! /usr/bin/env bash$
$ 
hostname$ 
```
