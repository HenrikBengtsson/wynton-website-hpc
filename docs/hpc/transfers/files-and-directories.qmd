...
For _storage_ mounted _directly_ on the {{< meta cluster.name >}} cluster, files can be transferred to and from the cluster using regular file copying, e.g. `cp` and `rsync`.
...
## <span class="glyphicon glyphicon-upload"></span> From your local machine to the {{< meta cluster.name >}} file system

### Copy a single file on your local file system to your {{< meta cluster.name >}} home directory

To copy a single file to your home directory (`~/`) on the cluster, use

```sh
{local}$ scp one_file.tsv alice@{{< meta transfer.hostname >}}:~/
```


### Copy one or more files to a folder on the cluster

To copy multiple files to {{< meta cluster.name >}} so they appear directly under `~/study/files/`, use

```sh
{local}$ scp *.txt *.R ../some/path/another_file.tsv alice@{{< meta transfer.hostname >}}:study/files/
```

### Recursively copy a directory to a folder on the cluster

To copy all content of directory `dataset/` to {{< meta cluster.name >}} so that it appears as `~/study/dataset/`, use

```sh
{local}$ scp -r dataset/ alice@{{< meta transfer.hostname >}}:study/
```



## <span class="glyphicon glyphicon-download"></span> From the {{< meta cluster.name >}} file system to your local machine

### Copy a single file from your {{< meta cluster.name >}} home directory to your local machine

To copy a single file in your  {{< meta cluster.name >}} home directory to the working directory of your local machine, use

```sh
{local}$ cd /path/project
{local}$ scp alice@{{< meta transfer.hostname >}}:one_file.tsv .
```
...
```sh
{local}$ scp alice@{{< meta transfer.hostname >}}:study/files/*.txt alice@{{< meta transfer.hostname >}}:study/files/*.R ~/study/
```
...
```sh
{local}$ cd /path/project
{local}$ scp -r alice@{{< meta transfer.hostname >}}:dataset/ .
```
