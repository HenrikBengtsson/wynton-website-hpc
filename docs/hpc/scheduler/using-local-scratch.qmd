...
* **Specify how much local scratch (TMPDIR) storage your job will need**.  Local storage is limited to [0.1-1.8 TiB/node](/hpc/about/specs.html).  If your job will use up to 200 GiB of disk space, you can specify this resource as `-l scratch=200G` (in units of GiB) when submitting the job.  A node with 800 GiB of scratch space can support up to four `-l scratch=200G` jobs running at the same time.
