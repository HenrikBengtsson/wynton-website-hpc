...
2022-12-20: We are still testing this feature. Although basic use-case testing has been done, Multi-Node Multi-GPU jobs still haven’t been thoroughly validated by end users. If you run into issues or have suggestions to make the documentation clearer, please reach out to [{{< meta cluster.email_support >}}](mailto::{{< meta cluster.email_support >}}).
</div>
...
The GPU nodes in {{< meta cluster.name >}} contain many different generations and models of NVIDIA GPUs.  In order to ensure that your GPU jobs run on GPUs with the proper capabilities, there are two SGE resource complexes assigned to each GPU node:
...
```sh
[alice@{{< meta devel.name >}} ~]$ qconf -se msg-iogpu3
hostname              msg-iogpu3
load_scaling          NONE
