...
**Normally, you would not set the email address for where SGE notifications are sent to.**  By default, the email notifications are sent to the email address you have associated with your {{< meta cluster.nickname >}} account.  To find out which address this is, run:
...
To send to multiple email addresses, use semicolon (`;`) to separate the addresses, e.g. `-M {{< meta user.email >}},alice2@bobson.org`.

_Technical details_: When not specifying `-M`, the `To:` address used for these notifications will be `<username>@<hostname>`, where `<hostname>` is a {{< meta cluster.nickname >}} hostname, e.g. `alice@log1.wynton.ucsf.edu`.  That is a local address that only exists on the {{< meta cluster.nickname >}} cluster. Any message sent to this address is redirected to your Wynton-associated email address.  If you configure SGE to use another email address (e.g. `-M alice2@bobson.org`), then that will become the `To:` address.
