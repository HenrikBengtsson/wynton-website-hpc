...
The {{< meta cluster.name >}} cluster farm consists of a large number of [compute nodes] that are ready to serve users' compute tasks (aka jobs).  Since all compute nodes are configured the same way, for instance, they have the exact same set software installed, it does not matter on which compute node your analysis runs.
...
The most common way of running compute tasks on the {{< meta cluster.name >}} cluster, consists of:
...
The {{< meta cluster.name >}} cluster uses [Son of Grid Engine] (SGE) as its scheduler.  SGE provides command `qsub` to submit a script ("job") and command `qstat` to check the status of a job.
...
```sh
[alice@{{< meta devel.name >}} ~]$ cd tests/
[alice@{{< meta devel.name >}} tests]$ chmod ugo+x hello_world
```

This, in combination with the so called "she-bang" (`#! ...`) on the first line, allows you call the script just any other software, e.g.

```sh
[alice@{{< meta devel.name >}} tests]$ ./hello_world
Hello world, I am running on node {{< meta devel.name >}}.
...
```sh
[alice@{{< meta devel.name >}} tests]$ qsub -cwd -j yes hello_world
...
```sh
[alice@{{< meta devel.name >}} tests]$ qstat
...
```sh
[alice@{{< meta devel.name >}} tests]$ cat hello_world.o201
Hello world, I am running on node cin-hmid1
Mon Aug 28 16:32:12 PDT 2017
[alice@{{< meta devel.name >}} tests]$ 
```
