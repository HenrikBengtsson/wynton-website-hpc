...
**Access to {{< meta cluster.name >}} from outside of the UCSF network requires two-factor authentication (2FA).**   If you connecting via the UCSF campus network, 2FA is _not_ required.  Likewise, if you are on the UCSF VPN, you are already fully authenticated on the campus network and _no_ further 2FA is needed to access {{< meta cluster.name >}}.  In all other cases, you will be prompted to authenticate through a Wynton-specific 2FA method when SSH:ing directly to the cluster.



## Requirements

Similarly to the UCSF VPN, {{< meta cluster.name >}} requires two-factor authentication via the Duo 2FA system.  Duo supports authentication via:
 ...
 * If you wish to change your 2FA method, phone number, or SMS number, you will need to contact the [{{< meta cluster.name >}} support staff] to reset your existing 2FA registration or resend the registration link.
 
 * If you receive a message from Duo that you, "have been locked out due to excessive authentication failures," you will need to contact the [{{< meta cluster.name >}} support staff] to have your Duo account re-enabled.

 * Support for hardware 2FA keys (e.g. YubiKey, Feitian, etc.) is limited and might not even work.  For example, when registering a hardware key, that will be your only option.  Also, if you have already registered your hardware 2FA key with UCSF (e.g. UCSF VPN), then that physical key can _not_ be used with the {{< meta cluster.nickname >}} 2FA system.  This is also true if the hardware key supports two or more security keys (e.g. short press and long press on a YubiKey).
...
After having completed the 2FA registration (see below) for your account, you can access {{< meta cluster.name >}} via SSH from outside the UCSF network.  A typical SSH login will then start by you authenticating yourself via 2FA as illustrated by:

```sh
{local}$ ssh alice@{{< meta login.hostname >}}
alice@{{< meta login.hostname >}} password:
...
[alice@{{< meta login.name >}}]$ 
```

For examples on what it looks like when you authenticate via other options, see the examples below.


## Registration for 2FA on {{< meta cluster.name >}}

In order to authenticate via 2FA, you will first have to register your Duo 2FA setup with {{< meta cluster.name >}}.  Since they are different 2FA systems, you have to complete this registration regardless whether or not you have already registered Duo 2FA for the UCSF VPN.  Below are detailed instructions on how to register 2FA for {{< meta cluster.name >}}.

_Comment_: If you are asked to 'Please contact your help or support desk' during the Duo 2FA registration, please contact the [{{< meta cluster.name >}} support staff] (do _not_ contact the UCSF IT Service Desk).


<div class="alert alert-warning" role="alert" style="margin-top: 3ex">
We recommend that you read through the below instructions before starting the registration. **You can only register once and you cannot reconfigure your 2FA setup afterwards**.  The only way to update the settings is to contact the {{< meta cluster.name >}} support staff to reset your 2FA setup.
</div>


### Step 1: Obtain a Duo registration link

In order to register with Duo 2FA, you need to obtain a registration link (URL) to your personal registration page.  This is done by attempting to log into {{< meta cluster.nickname >}} via SSH.

If this is the first time you access {{< meta cluster.name >}} via SSH, then you will have to do two SSH logins - the first login is just a "trigger" and the second login one will display the registration URL.  If you have priorly logged into to {{< meta cluster.name >}}, then you can skip to the second SSH-login instructions below.

1. SSH to {{< meta login.hostname >}} using your {{< meta cluster.nickname >}} username
   - If you are asked the question `Are you sure you want to continue connecting (yes/no/[fingerprint])?`, then answer `yes`
   - Enter your {{< meta cluster.nickname >}} password
   - The connection will be closed automatically. This is expected

2. Again, SSH to {{< meta login.hostname >}} using your {{< meta cluster.nickname >}} username
   - Enter your {{< meta cluster.nickname >}} password
...
```sh
{local}$ ssh alice@{{< meta login.hostname >}}
The authenticity of host {{< meta login.hostname >}} ({{< meta login.ip >}}) can not be established.
ECDSA key fingerprint is SHA256:DrCbFJouT3pRHoPO6rzGNJxX4OOIBuLy/ZdxjIQrx3M.
Are you sure you want to continue connecting (yes/no/[fingerprint])? yes
Warning: Permanently added {{< meta login.hostname >}},{{< meta login.ip >}} (ECDSA) to the list of known hosts.
Connection closed by {{< meta login.ip >}} port 22
...
alice@{{< meta login.hostname >}}: Permission denied (publickey,gssapi-with-mic,keyboard-interactive).
```
...
1. Select the type of device you are using;
   ...
   - We do _not_ recommend using 'Security Key' because then you will not be able to use the 'Mobile phone' options.  Also, if you have already registered your 'Security Key' for the UCSF VPN, it will _not_ work with the {{< meta cluster.name >}} 2FA.
...
After having completed the above 2FA registration, you should be able to access {{< meta cluster.name >}} via SSH.  As illustrated below, verify that this works by SSH:ing to a {{< meta cluster.name >}} login or data-transfer node using your {{< meta cluster.nickname >}} username and:

1. Enter your {{< meta cluster.nickname >}} password
...
[{{< meta cluster.name >}} support staff]: /hpc/support/index.html
