...
Although you should always run analyses via the job scheduler, there are times when you may need to develop parts of it interactively at the command-line prompt.  For instance, you may need to install some software, a few R packages, or run some quick tests on your new pipeline.  {{< meta cluster.name >}} provides _development nodes_ dedicated for such short-term usages and that are configured similarly to the compute nodes.
...
<div class="non-protected">
 * {{< meta dev1.hostname >}}
 * {{< meta dev2.hostname >}}
 * {{< meta dev3.hostname >}}
 * {{< meta gpudev1.hostname >}}
</div>
<div class="protected">
The following development nodes are available to Wynton Protected users:

 * p{{< meta dev1.hostname >}}
 * p{{< meta gpudev1.hostname >}}
...
To access one of the _development node_, say, {{< meta devel.hostname >}},

1. make sure you are [logged in to the cluster], and

2. type `ssh {{< meta devel.hostname >}}` and press <kbd>ENTER</kbd>.
...
```sh
[alice@{{< meta login.name >}} ~]$ ssh {{< meta devel.hostname >}}
alice1@{{< meta devel.name >}}:s password: XXXXXXXXXXXXXXXXXXX
