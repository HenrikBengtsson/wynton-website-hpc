...
<div id="wifi-alert" class="alert alert-warning" role="alert" style="margin-top: 3ex; font-size: 200%; display: none">
...
by SSH.** The symptom is an "ssh: connect to host {{< meta login.hostname >}}
...
**Access to {{< meta cluster.name >}} from outside of the UCSF network requires two-factor authentication (2FA).**
...
To access the {{< meta cluster.name >}} cluster you need an account - any UCSF affiliate can get an [account](/hpc/about/join.html).  Anyone with a {{< meta cluster.name >}} account, can access the cluster via _Secure Shell (SSH)_.
...
* have a {{< meta cluster.name >}} account _(not the same as your UCSF
  account)_

* know your {{< meta cluster.name >}} username _(not the same as your
  UCSF password)_

* know your {{< meta cluster.name >}} password _(not the same as your
  UCSF password)_

* know whether you have a Regular or Protected {{< meta cluster.name >}}
...
<div class="alert alert-warning" role="alert" style="margin-top: 3ex; font-size: 150%;">
...
**If this is the first time you access {{< meta cluster.name >}} and you are outside of the UCSF network, we recommend that you first log onto the UCSF VPN before continuing below.**  This avoids having to deal with {{< meta cluster.nickname >}}'s two-factor authentication that is otherwise required when accessing the cluster from outside of the UCSF.  Note that this advice is just to get you started.  In the long run, you do not want to be on the very bandwidth-limited UCSF VPN if you are transferring large amounts of files to and from Wynton to your local machine.
</div>

To log into the cluster, assuming your user name on {{< meta cluster.name >}} is `alice` (case sensitive), do the following:
...
3. at the terminal prompt, type `ssh alice@{{< meta login.hostname >}}` and press <kbd>ENTER</kbd>, and

4. when prompted, enter your {{< meta cluster.name >}} password.
...
alice@{{< meta login.hostname >}}:s password: 
...
If you get `Permission denied, please try again.` when you enter your password, make sure you use the correct {{< meta cluster.name >}} username (case sensitive) and the correct password.
...
Like for all Unix-based system, to log out from {{< meta cluster.name >}}, type `exit` or `logout` (standard Unix commands) and press <kbd>ENTER</kbd>.  For example,

```sh
[alice@{{< meta login.name >}} ~]$ exit
Connection to {{< meta login.hostname >}} closed.
{local}$ 
```
