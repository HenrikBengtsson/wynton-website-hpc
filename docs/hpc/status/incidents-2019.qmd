...
**Notice**: It is not possible to log in to the {{< meta cluster.name >}} environment.  The reason is currently not known.
...
**Resolved**: The {{< meta cluster.name >}} environment is fully functional again. The BeeGFS filesystem was not working properly during 18:30-22:10 on December 18 resulting in no login access to the cluster and job file I/O being backed up.
...
**Notice**: One of two network switches will be upgraded on Wednesday December 18 starting at 18:00 and lasting a few hours.  We do not expect this to impact the {{< meta cluster.name >}} environment other than slowing down the network performance to 50%.
...
**Notice**: It is not possible to log in to the {{< meta cluster.name >}} environment.  This is due to a recent misconfiguration of the LDAP server.
...
**Resolved**: The {{< meta cluster.name >}} BeeGFS file system is fully functional again.  During the outage, `/wynton/group` and `/wynton/scratch` was not working properly, whereas `/wynton/home` was unaffected.
<br><span class="timestamp">October 23, 10:35 PT</span>

**Notice**: The {{< meta cluster.name >}} BeeGFS file system is non-functional.  It is expected to be resolved by noon on October 23. The underlying problem is that the power backup at the Diller data center did not work as expected during a planned power maintenance.
...
**Resolved**: The {{< meta cluster.name >}} environment is up and running again.
<br><span class="timestamp">September 24, 20:25 PT</span>

**Notice**: The {{< meta cluster.name >}} environment is unresponsive.  Problem is being investigated.
...
**Resolved**: The {{< meta cluster.name >}} environment is up and running again. The reason for this downtime was the BeeGFS file server became unresponsive.
<br><span class="timestamp">August 23, 20:45 PT</span>

**Notice**: The {{< meta cluster.name >}} environment is unresponsive.
...
**Notice**: The {{< meta cluster.name >}} environment is down due to a non-planned power outage at the Diller data center.  Jobs running on compute nodes located in that data center, were terminated.  Jobs running elsewhere may also have been affected because `/wynton/home` went down as well (despite it being mirrored).
...
**Notice**: The {{< meta cluster.name >}} environment is down due to a non-planned power outage at the main data center.
...
**Resolved**: The {{< meta cluster.name >}} environment and the BeeGFS file system are fully functional after updates and upgrades.
<br><span class="timestamp">July 12, 11:15 PT</span>

**Notice**: The {{< meta cluster.name >}} environment is down for maintenance.
<br><span class="timestamp">July 8, 12:00 PT</span>

**Notice**: Updates to the BeeGFS file system and the operating system that require to bring down all of {{< meta cluster.name >}} will start on the morning of Monday July 8.  Please make sure to log out before then.  The downtime might last the full week.
...
**Investigating**: There is a major slowdown of the BeeGFS file system (`/wynton`), which in turn causes significant problems throughout the {{< meta cluster.name >}} environment.
...
**Investigating**: The BeeGFS file system (`/wynton`) is experiencing major issues.  This caused all on {{< meta cluster.name >}} to become non-functional.
...
**Resolved**: The UCSF-wide network issue that affected access to {{< meta cluster.name >}} has been resolved.
...
**Investigating**: There are issues logging in to {{< meta cluster.name >}}.
