{{< include warning-hold-on-storage-requests.qmd >}}


# Pricing for Extra Storage

Research groups can purchase additional storage on {{< meta cluster.nickname >}} beyond the user-specific storage that is free to all.
...
* To purchase more storage, please contact [{{< meta cluster.email_storage >}}](mailto:{{< meta cluster.email_storage >}}?subject=Request%20to%20purchase%20dedicated%20storage)
...
* Unlike nodes, the storage cannot be extracted from the system once it’s been brought online. Once a lab or a group has “bought in”, they will not be able to retrieve their portion of storage hardware if they choose to leave {{< meta cluster.nickname >}} (unlike compute nodes/shares).
...
We have an immediate need to provide for {{< meta cluster.name >}} storage expansion to meet the demands of various research groups on campus. Given the difficulty of predicting longer-term costs and issues, the current pricing is considered short-term and may change as we understand the evolving needs and operational realities of {{< meta cluster.name >}}. This model is based on some assumptions that are important to understand:
