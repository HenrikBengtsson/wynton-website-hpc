...
<dl id="hosttable-summary" class="dl-horizontal">
  <dt>Compute nodes</dt><dd id="hosttable-summary-nodes">482</dd>
  <dt>Physical cores</dt><dd id="hosttable-summary-cores">17496</dd>
  <dt>GPUs</dt><dd>235 GPUs on 61 GPU nodes (88/22 GPUs/nodes are communal and 147/39 GPUs/nodes are prioritized for GPU contributors)</dd>
  <dt>RAM</dt><dd id="hosttable-summary-ram">48-1512 GiB/node</dd>
  <dt>Local scratch</dt><dd id="hosttable-summary-local-scratch">0.1-1.8 TiB/node</dd>
  <dt>Global scratch</dt><dd id="hosttable-summary-global-scratch">703 TiB</dd>
  <dt>User home storage</dt><dd>500 GiB/user (770 TiB in total)</dd>
  <dt>Group storage</dt><dd>6.5 PB</dd>
  <dt>Number of accounts</dt><dd><a href="https://ucsf-wynton.github.io/wyntonquery/reference/users_over_time.html">1312</a> of which 365 are Wynton Protected accounts</dd>
  <dt>Number of projects</dt><dd>761</dd>
</dl>
...
1. `{{< meta login1.hostname >}}`
2. `{{< meta login2.hostname >}}`
1. `p{{< meta login1.hostname >}}` (for Wynton Protected users)
...
1. `{{< meta transfer1.hostname >}}`
2. `{{< meta transfer2.hostname >}}`
1. `p{{< meta transfer1.hostname >}}` (for Wynton Protected users)
2. `p{{< meta transfer2.hostname >}}` (for Wynton Protected users)
...
-----------------------------------|---------------:|---------:|-----------------:|-----------------:|------------------------------:|-------------------:|
`{{< meta dev1.hostname >}}`         |             72 |  384 GiB |         0.93 TiB | x86-64-v4        | Intel Gold 6240 2.60GHz       |                    |
`{{< meta dev2.hostname >}}`         |             48 |  512 GiB |         0.73 TiB | x86-64-v3        | Intel Xeon E5-2680 v3 2.50GHz |                    |
`{{< meta dev3.hostname >}}`         |             48 |  256 GiB |         0.73 TiB | x86-64-v3        | Intel Xeon E5-2680 v3 2.50GHz |                    |
`{{< meta gpudev1.hostname >}}`      |             56 |  256 GiB |         0.82 TiB | x86-64-v3        | Intel Xeon E5-2660 v4 2.00GHz | NVIDIA GeForce GTX 1080 |
`p{{< meta dev1.hostname >}}` (for Wynton Protected users)    |  32 | 256 GiB |         1.1  TiB | x86-64-v3        | Intel E5-2640 v3              |                    |
`p{{< meta gpudev1.hostname >}}` (for Wynton Protected users) | 56 |  256 GiB |         0.82 TiB | x86-64-v3        | Intel Xeon E5-2660 v4 2.00GHz | NVIDIA GeForce GTX 1080 |
...
<!--
Source: `nproc --all`, `free -ht`, `df --block-size=1 /scratch`, `lscpu`, and `lspci | grep -i -E "(3d|nvidia)"`
-->
...
The {{< meta cluster.name >}} cluster provides two types of scratch storage:

 * Local `/scratch/` - <span id="hosttable-summary-local-scratch2">0.1-1.8 TiB/node</span> storage unique to each compute node (can only be accessed from the specific compute node).
 * Global `/wynton/scratch/` and `/wynton/protected/scratch/` (for Wynton Protected users) - 703 TiB storage ([BeeGFS](https://www.beegfs.io/content/)) accessible from everywhere.
...
Importantly, note that **the {{< meta cluster.name >}} storage is not backed up**.  Users and labs are responsible for backing up their own data outside of {{< meta cluster.name >}}.
</div>
