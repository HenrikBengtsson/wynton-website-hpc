...
* Migration: The UCSF Memory and Aging Center (MAC) have joined {{< meta cluster.name >}} by contributing new nodes

* Migration: The UCSF Macromolecular Structure Group (MSG) compute cluster (~1,300 physical cores) is incorporated into {{< meta cluster.name >}}

* Development: Dedicated 10 Gbps transfer node for faster file transfers in to and out from {{< meta cluster.name >}} (2018-08-02)
...
* Scheduler: ~~Support interactive jobs (`qlogin`)~~. The {{< meta cluster.name >}} Team decided against this because it increases the risk of having stale jobs occupying the scheduler and overall underutilizing the HPC environment (Fall 2019)

* Network: A second data transfer node for faster file transfers in to and out from {{< meta cluster.name >}} (2019-09-12)
...
* Website: Move website from GitHub Pages (https&#58;//ucsf-hpc.github.io/wynton/) to on-premise <{{< meta organization.url >}}/hpc/> (2020-02-26)

* Network: Add a Globus '{{< meta cluster.nickname >}}' Endpoint for efficient, secure data transfers (ingress and egress) (2020-04-03)

* Access: Two-factor authentication (2FA) is required for accessing the cluster (2020-10-29)

* Compute: QB3 hardware migration to {{< meta cluster.name >}} considered complete (approximately 5,500 out of 7,000 physical cores migrated) (2021-09-27)
...
* Network: Support for using Globus transfers to and from UCSF Box via {{< meta cluster.nickname >}} (2022-06-01)
