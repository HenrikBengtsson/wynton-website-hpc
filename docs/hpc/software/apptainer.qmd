...
The {{< meta cluster.name >}} cluster supports [Linux containers] via the [Apptainer] software (formerly known as [Singularity]).  A Linux container is an efficient and powerful virtualization method for running isolated Linux systems ("containers") on any Linux system including the one used by our cluster.
...
All tasks for using Linux containers, such as downloading, building, and running containers, is done via the `apptainer` client and supported on {{< meta cluster.name >}}.  The most common command calls are:
...
* Install Apptainer or Docker on your own machine:
  - Create a Apptainer image on your own workstation and transfer the image to {{< meta cluster.name >}}
  - Create a Docker image on your own workstation and transfer the image to {{< meta cluster.name >}}
...
```sh
[alice@{{< meta devel.name >}} ~]$ mkdir lxc
mkdir: cannot create directory ‘lxc’: File exists
[alice@{{< meta devel.name >}} ~]$ cd lxc/
[alice@{{< meta devel.name >}} lxc]$ apptainer build rocker_r-base.sif docker://rocker/r-base
INFO:    Starting build...
...
[alice@{{< meta devel.name >}} lxc]$ ls -l rocker_r-base.sif
...
```sh
[alice@{{< meta devel.name >}} lxc]$ apptainer run rocker_r-base.sif
...
```sh
[alice@{{< meta devel.name >}} lxc]$ apptainer exec rocker_r-base.sif R --quiet
> sum(1:10)
[1] 55
> q("no")
[alice@{{< meta devel.name >}} lxc]$ 
...
```sh
[alice@{{< meta devel.name >}} lxc]$ ./rocker_r-base.sif
...
To launch a shell within this container, and to also convince yourselves that the container runs Ubuntu (and not Rocky 8 as on the {{< meta cluster.name >}} host system), do:

<!-- code-block label="shell" -->
```sh
[alice@{{< meta devel.name >}} lxc]$ apptainer shell rocker_r-base.sif
...
[alice@{{< meta devel.name >}} lxc]$ head -3 /etc/os-release
NAME="Rocky Linux"
...
When running a container, only a few of the folders available "outside" are available "inside" the container.  By default, you have access to the current working directory (= `$PWD`) and your home folder (= `$HOME`).   In contrast, without further specifications, you will not have access to standard folders such as local `/scratch` and global `{{< meta path.global_scratch >}}`.  Similarly, lab folders such as `{{< meta user.labfolder >}}` are not available from inside the container.
...
Apptainer> ls {{< meta path.global_scratch >}}
ls: cannot access '{{< meta path.global_scratch >}}': No such file or directory
Apptainer> ls {{< meta user.labfolder >}}
ls: cannot access '{{< meta user.labfolder >}}': No such file or directory
...
```sh
[alice@{{< meta devel.name >}} lxc]$ apptainer shell --bind /scratch,{{< meta path.global_scratch >}},{{< meta user.labfolder >}} rocker_r-base.sif
Apptainer> echo "$TMPDIR"
/scratch/alice
Apptainer> ls -dF "$TMPDIR"
/scratch/alice/
...
Apptainer> ls -dF {{< meta path.global_scratch >}}/alice
...
```sh
[alice@{{< meta devel.name >}} lxc]$ mkdir -p extra/logs
[alice@{{< meta devel.name >}} lxc]$ echo "Hello world" > extra/logs/hello.txt
[alice@{{< meta devel.name >}} lxc]$ apptainer shell --bind extra/logs:/var/log rocker_r-base.sif
Apptainer> date > /var/log/timestamp
Apptainer> ls /var/log
hello.txt  timestamp
Apptainer> exit
exit
[alice@{{< meta devel.name >}} lxc]$ ls extra/logs
hello.txt  timestamp
```
...
```sh
[alice@{{< meta devel.name >}} lxc]$ apptainer exec "rocker_r-base.sif" Rscript -e "sum(1:10)"
[1] 55
[alice@{{< meta devel.name >}} lxc]$ 
```
...
```sh
[alice@{{< meta devel.name >}} lxc]$ cat demo-apptainer.sh
#!/usr/bin/bash
#$ -S /bin/bash
#$ -cwd
#$ -j y
...
```sh
[alice@{{< meta devel.name >}} lxc]$ qsub demo-apptainer.sh
...
```sh
[alice@{{< meta devel.name >}} lxc]$ cat demo-apptainer.o5987
[1] 55
```
...
also from scratch from one of the {{< meta cluster.name >}} development
nodes.
...
mkdir: cannot create directory ‘lxc’: File exists
[alice@{{< meta devel.name >}} ~]$ cd lxc/
[alice@{{< meta devel.name >}} lxc]$ apptainer build isoseq3.sif isoseq3.def
...
```sh
[alice@{{< meta devel.name >}} lxc]$ ls -l isoseq3.sif
...
```sh
[alice@{{< meta devel.name >}} lxc]$ ./isoseq3.sif --version
