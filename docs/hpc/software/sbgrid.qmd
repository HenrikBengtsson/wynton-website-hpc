...
Access to SBGrid software on {{< meta cluster.name >}} is limited to
paying members of the SBGrid Consortium.
</div>

<div class="alert alert-warning" role="alert" style="margin-top: 3ex" markdown="1">
For bug reports related to SBGrid software, please use the [SBGrid's
Report Software Bug form] The {{< meta cluster.nickname >}} sysadmins do
...
without needing intervention by the {{< meta cluster.nickname >}} admins.
Each lab using SBGrid must pay a yearly fee. This covers usage by anyone
in the lab, both on {{< meta cluster.name >}} and any other Linux or macOS
...
The SBGrid software stack is installed on {{< meta cluster.name >}}, but
is available only to paying members, which are listed on the
[SBGrid Member Labs] webpage.  As a {{< meta cluster.name >}} user part
of a paying lab, you will have access to SBGrid on the
{{< meta cluster.name >}} cluster from being part of the `sbgrid` Unix
group.  You can verify that this is get case by running:

<!-- code-block label="groups" -->
```sh
[alice@{{< meta devel.name >}} ~]$ groups
boblab sbgrid
```

If `sbgrid` is not part of your output, then you currently do _not_
have access to the SBGrid software on {{< meta cluster.name >}}.  If you
...
```sh
[alice@{{< meta devel.name >}} ~]$ source /programs/sbgrid.shrc
********************************************************************************
...
```sh
[alice@{{< meta devel.name >}} ~]$ sbgrid-info -l relion
...
```sh
[alice@{{< meta devel.name >}} ~]$ relion --version
