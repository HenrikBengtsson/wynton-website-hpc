...
```sh
[alice@{{< meta devel.name >}} ~]$ module load CBI rstudio-server-controller
[alice@{{< meta devel.name >}} ~]$ rsc start
...
to {{< meta devel.name >}}, you need to set up SSH port forwarding first, which you can do by
running:

  ssh -L 20612:{{< meta devel.name >}}:20612 alice@{{< meta login.hostname >}}
...
```sh
{local}$ ssh -L 20612:{{ site.devel.hostname}}:20612 alice@{{< meta login.hostname >}}
...
### ERROR: Failed to check process PID 12345 on {{< meta dev1.hostname >}} over SSH

If you get the following error when launching `rsc start`:

```sh
[alice@{{ site.dev2.name }} ~]$ rsc start
WARNING: Needs to SSH to {{< meta dev1.hostname >}} to check whether process 2132343
is still alive [{{< meta user.home >}}/.config/rsc/rserver.hostname:
...
{{< meta dev2.hostname >}}
ERROR: Failed to check process PID 2132343 on {{< meta dev1.hostname >}} over SSH. 
Reason was: ssh: connect to host {{< meta dev1.hostname >}} port 22: No route to host
```
