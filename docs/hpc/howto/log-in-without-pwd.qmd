...
```sh
{local}$ ssh-copy-id -i ~/.ssh/laptop_to_{{ site.cluster.nickname | downcase }}.pub alice@{{< meta login.hostname >}}
/usr/bin/ssh-copy-id: INFO: Source of key(s) to be installed: "{{< meta user.home >}}/.ssh/laptop_to_{{ site.cluster.nickname | downcase }}.pub"
...
Now try logging into the machine, with:   "ssh 'alice@{{< meta login.hostname >}}'"
...
```sh
{local}$ scp ~/.ssh/laptop_to_{{ site.cluster.nickname | downcase }}.pub alice@{{< meta login.hostname >}}:.ssh/
...
```sh
{local}$ ssh -o PreferredAuthentications=keyboard-interactive,password alice@{{< meta login.hostname >}}
alice1@{{< meta login.ip >}}\'s password: XXXXXXXXXXXXXXXXXXX
[alice@{{< meta login.name >}} ~]$ cd .ssh
[alice@{{< meta login.name >}} .ssh]$ cat laptop_to_{{ site.cluster.nickname | downcase }}.pub >> authorized_keys
```

Finally, make sure that `~/.ssh/authorized_keys` on {{< meta cluster.nickname >}} is only accessible to you (otherwise that file will be completely ignored by SSH);

```sh
[alice@{{< meta login.name >}} .ssh]$ chmod u=rw,go= ~/.ssh/authorized_keys
[alice@{{< meta login.name >}} .ssh]$ stat --format=%A ~/.ssh/authorized_keys
-rw-------
```
...
```sh
[alice@{{< meta login.name >}} .ssh]$ exit
{local}$ 
```
...
```sh
{local}$ ssh -o PreferredAuthentications=publickey,keyboard-interactive -o IdentitiesOnly=yes -i ~/.ssh/laptop_to_{{ site.cluster.nickname | downcase }} alice@{{< meta login.hostname >}}
...
```sh
{local}$ ssh -i ~/.ssh/laptop_to_{{ site.cluster.nickname | downcase }} alice@{{< meta login.hostname >}}
[alice@{{< meta login.name >}} ~]$ 
```
...
`alice@{{< meta login.hostname >}}`.  On your local machine, add the
following directive to `~/.ssh/config` (if you don't have the file, create
it):

```lang-none
Host *.{{< meta cluster.domain >}}
  User alice
...
```sh
{local}$ ssh {{< meta login.hostname >}}
[alice@{{< meta login.name >}} ~]$ 
```

Because we use globbing for `Host` in the above SSH config directive, it
applies to SSH connections for _all_ {{< meta cluster.nickname >}}
connections.
...
`Host *.{{< meta cluster.domain >}}` in `~/.ssh/config` on your 
local computer:
...
`Host: *.{{< meta cluster.domain >}}` directive in `~/.ssh/config`
on your local computer and verified that it  works.  Then, append
another directive with:

```lang-none
Host *dev?.{{< meta cluster.domain >}}
  ProxyJump {{< meta login.hostname >}}
```
...
```sh
{local}$ ssh {{< meta devel.name >}}.{{< meta cluster.domain >}}
[alice@{{< meta devel.name >}} ~]$ 
```

This works, because the `ProxyJump` specification makes the SSH
connection use `{{< meta login.hostname >}}` as a "jump host" and from
...
have called `ssh -J {{< meta login.hostname >}} {{< meta devel.name >}}.{{< meta cluster.domain >}}`.
