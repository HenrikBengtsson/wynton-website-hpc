...
```sh
[alice@{{< meta devel.name >}} ~]$ python3 -m pip install --user notebook
```

## Running Jupyter Notebook

[Jupyter Notebook] should only be run on the {{< meta cluster.name >}}
development nodes. However, you cannot connect from outside
{{< meta cluster.name >}} directly to a development node, but rather
...
Running Jupyter Notebook on {{< meta cluster.name >}} involves three
...
```sh
[alice@{{< meta devel.name >}} ~]$ module load CBI port4me
[alice@{{< meta devel.name >}} ~]$ port4me --tool=jupyter
47467
```
...
```sh
[alice@{{< meta devel.name >}}]$ jupyter notebook --no-browser --port 47467
...
[I 2024-03-20 14:48:46.675 LabApp] JupyterLab extension loaded from {{< meta user.home >}}/.local/lib/python3.11/site-packages/jupyterlab
[I 2024-03-20 14:48:46.675 LabApp] JupyterLab application directory is {{< meta user.home >}}/.local/share/jupyter/lab
...
[I 2024-03-20 14:48:46.712 ServerApp] Serving notebooks from local directory: {{< meta user.home >}}
...
[C 2024-03-20 14:48:46.725 ServerApp]
    ...
    To access the server, open this file in a browser:
        file://{{< meta user.home >}}/.local/share/jupyter/runtime/jpserver-2853162-open.html
...
runs_, which in our case is development node `{{< meta devel.name >}}`.
...
```sh
{local}$ ssh -J alice@{{< meta login.hostname >}} -L 47467:localhost:47467 alice@{{< meta devel.name >}}
...
...
`{{< meta devel.name >}}` is accessible directly from the web
...
X2Go via one of the {{< meta cluster.name >}} development nodes.


<div class="alert alert-info" role="alert" markdown="1">
If you use Python via your own Conda installation, instead of the
Python version provided by {{< meta cluster.name >}}, you can launch a
