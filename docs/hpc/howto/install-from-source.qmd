...
```sh
[alice@{{< meta devel.name >}} ~]$ mkdir -p "/scratch/$USER"
[alice@{{< meta devel.name >}} ~]$ cd "/scratch/$USER"
[alice@{{< meta devel.name >}} alice]$ wget https://github.com/samtools/samtools/releases/download/1.14/samtools-1.14.tar.bz2
[alice@{{< meta devel.name >}} alice]$ ls -l samtools-1.14.tar.bz2 
...
```sh
[alice@{{< meta devel.name >}} alice]$ tar -x -f samtools-1.14.tar.bz2
[alice@{{< meta devel.name >}} alice]$ ls -l
total 7568
...
```sh
[alice@{{< meta devel.name >}} alice]$ cd samtools-1.14
[alice@{{< meta devel.name >}} samtools-1.14]$ ls
...
```sh
[alice@{{< meta devel.name >}} samtools-1.14]$ mkdir -p $HOME/software/samtools-1.14
```

Now we can configure the build to install to this folder:

```sh
[alice@{{< meta devel.name >}} samtools-1.14]$ ./configure --prefix=$HOME/software/samtools-1.14
configure: running /bin/sh ./configure --disable-option-checking '--prefix={{< meta user.home >}}/software/samtools-1.14'  --cache-file=/dev/null --srcdir=.
...
[alice@{{< meta devel.name >}} samtools-1.14]$  
```
...
```sh
[alice@{{< meta devel.name >}} samtools-1.14]$ make
...
...
```sh
[alice@{{< meta devel.name >}} samtools-1.14]$ make install
mkdir -p -m 755 {{< meta user.home >}}/software/samtools-1.14/bin {{< meta user.home >}}/software/samtools-1.14/bin {{< meta user.home >}}/software/samtools-1.14/share/man/man1
install -p samtools {{< meta user.home >}}/software/samtools-1.14/bin
install -p misc/ace2sam misc/maq2sam-long misc/maq2sam-short misc/md5fa misc/md5sum-lite misc/wgsim {{< meta user.home >}}/software/samtools-1.14/bin
install -p misc/blast2sam.pl misc/bowtie2sam.pl misc/export2sam.pl misc/fasta-sanitize.pl misc/interpolate_sam.pl misc/novo2sam.pl misc/plot-ampliconstats misc/plot-bamstats misc/psl2sam.pl misc/sam2vcf.pl misc/samtools.pl misc/seq_cache_populate.pl misc/soap2sam.pl misc/wgsim_eval.pl misc/zoom2sam.pl {{< meta user.home >}}/software/samtools-1.14/bin
install -p -m 644 doc/samtools*.1 misc/wgsim.1 {{< meta user.home >}}/software/samtools-1.14/share/man/man1
[alice@{{< meta devel.name >}} samtools-1.14]$ 
```
...
```sh
[alice@{{< meta devel.name >}} samtools-1.14]$ $HOME/software/samtools-1.14/bin/samtools --version | head -3
samtools 1.14
Using htslib 1.14
...
```sh
[alice@{{< meta devel.name >}} ~]$ export PATH=~/software/samtools-1.14/bin:$PATH
[alice@{{< meta devel.name >}} samtools-1.14]$ which samtools
~/software/samtools-1.14/bin/samtools
[alice@{{< meta devel.name >}} samtools-1.14]$ samtools --version | head -3
samtools 1.14
Using htslib 1.14
Copyright (C) 2021 Genome Research Ltd.
```
...
```sh
[alice@{{< meta devel.name >}} ~]$ gcc --version
...
```sh
[alice@{{< meta devel.name >}} ~]$ module load CBI scl-gcc-toolset/12
[alice@{{< meta devel.name >}} ~]$ gcc --version
