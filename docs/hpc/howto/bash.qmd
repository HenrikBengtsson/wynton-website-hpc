...
Unless you have explicitly requested to use a different shell than Bash, you will be running a Bash shell when you log onto any of {{< meta cluster.name >}}'s machines.  You can confirm this by looking at the value of the `SHELL` environment variable;

<!-- code-block label="bash-SHELL" -->
```sh
[alice@{{< meta devel.name >}} ~]$ echo "$SHELL"
/bin/bash
```
...
```sh
[alice@{{< meta devel.name >}} ~]$ cat /etc/skel/.bashrc
# .bashrc
