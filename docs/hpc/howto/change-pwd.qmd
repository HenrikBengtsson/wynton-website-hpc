...
2. To access the page, enter your current {{< meta cluster.name >}}
...
After waiting for 10 minutes for your new password to propagate, you can verify your {{< meta cluster.nickname >}} username and password using either of the below alternatives.  If neither works for you, you might have to reset your password.  If so, see below for instructions.


### Alt 1. Verify from your local command line using SSH

To test your {{< meta cluster.nickname >}} credentials, try to [login to {{< meta cluster.name >}} via SSH](/hpc/get-started/access-cluster.html).  If you have SSH keys set up, you can force SSH to ignore those and only accept password entries by using:

```r
{local}$ ssh -o PreferredAuthentications=password alice@{{< meta login.hostname >}}
```
...
2. Enter your {{< meta cluster.nickname >}} login credentials in the two fields 'Username'  (e.g. `alice`) and 'Password' and click 'Sign in'.
...
If your _{{< meta cluster.nickname >}}_ password has expired, or you
