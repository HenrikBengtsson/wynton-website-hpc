...
The {{< meta cluster.name >}} environment supports running a graphical user interface (GUI) on {{< meta cluster.name >}} while viewing and interacting with it on your local computer.  More specifically, and in more technical terms, {{< meta cluster.name >}} supports NX and X11 Forwarding protocols.
...
Due to limitation in X2Go Client, it is _not_ possible to connect to {{< meta cluster.name >}}  _when using SSH password_(\*).  Instead, in order to connect to {{< meta cluster.nickname >}} using the X2Go Client, you have to have a working [SSH key pair] set up and configured the X2Go Client. 

If you are connecting to {{< meta cluster.nickname >}} from off campus, you will also have to use Two Factor Authentication (2FA) for {{< meta cluster.nickname >}}.  This is _not_ needed if you are connected via the UCSF VPN.
...
First, you will need to install the X2Go Client on your local computer.  For instructions on how to do this, see [Installing X2Go Client](https://wiki.x2go.org/doku.php/doc:installation:x2goclient). When you first run x2goclient:
 ...
 2. Pick a **Session name**, e.g. `{{< meta cluster.name >}}`
 ...
 4. Set the **Login** to _your_ {{< meta cluster.name >}} username, e.g. `alice`
 ...
 8. Then in the **Proxy server** section:
    ...
    - Set **Host** to a {{< meta cluster.name >}} [login node]: <span class="non-protected">`{{< meta login1.hostname >}}` or `{{< meta login2.hostname >}}`</span><span class="protected">`p{{< meta login1.hostname >}}`</span>
...
6. Dialog **MATE on dev2**: _Enter your {{< meta cluster.name >}} password_ (this happens for unknown reasons to _some_ users, even when using SSH keys)
...
You can also use X11 forwarding over the SSH connection used to connect to {{< meta cluster.name >}}.
...
```sh
{local}$ ssh -X -C alice@{{< meta login.hostname >}}
alice1@{{< meta login.hostname >}}:s password: 
[alice@{{< meta login.name >}} ~]$ echo "DISPLAY='$DISPLAY'"
...
```sh
[alice@{{< meta login.name >}} ~]$ ssh -X {{< meta devel.hostname >}}
alice1@{{< meta devel.name >}}:s password: 
[alice@{{< meta devel.name >}} ~]$ echo "DISPLAY='$DISPLAY'"
...
Now we have an X11 forward setup that runs all the way back to our local computer.  This will allow us to open, for instance, an XTerm window that runs on {{< meta devel.hostname >}} but can be interacted with on the local computer;

```sh
[alice@{{< meta devel.name >}} ~]$ xterm
...
```sh
{local}$ ssh -X -C -J alice@{{< meta login.hostname >}} alice@{{< meta devel.hostname >}}
[alice@{{< meta devel.name >}} ~]$ 
```
