...
```sh
[alice@{{< meta devel.name >}} ~]$ ls -l rocker_r-base.sif
...
```sh
[alice@{{< meta devel.name >}} ~]$ ls -s --block-size 1 rocker_r-base.sif
...
```sh
[alice@{{< meta devel.name >}} ~]$ du --apparent-size --block-size=1 rocker_r-base.sif
...
```sh
[alice@{{< meta devel.name >}} ~]$ du --block-size=1 rocker_r-base.sif
256136704       rocker_r-base.sif
```
...
```sh
[alice@{{< meta devel.name >}} ~]$ beegfs-ctl --getquota --storagepoolid=11 --uid "$USER"
...
```sh
[alice@{{< meta devel.name >}} ~]$ beegfs-ctl --getquota --storagepoolid=12 --gid "$(id --group)"
      user/group     ||           size          ||    chunk files
     name     |  id  ||    used    |    hard    ||  used   |  hard
--------------|------||------------|------------||---------|---------
...
```sh
[alice@{{< meta devel.name >}} ~]$ id --group
boblab
```
...
```sh
[alice@{{< meta devel.name >}} ~]$ id --groups
boblab carol_inst
```
...
```sh
[alice@{{< meta devel.name >}} ~]$ for group in $(id --groups); do beegfs-ctl --getquota --storagepoolid=12 --gid "$group"; done
      user/group     ||           size          ||    chunk files
     name     |  id  ||    used    |    hard    ||  used   |  hard
--------------|------||------------|------------||---------|---------
        boblab| 34001||   13.43 TiB|   40.00 TiB||        0|unlimited
