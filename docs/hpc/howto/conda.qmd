...
**Note**: The aim of this document is to give the essential best-practices for working with Conda on {{< meta cluster.nickname >}}. It is not meant to be a complete introduction on how to work with Conda in general. A good complement to this document, is the official Conda documentation on [Managing environments].


## Loading Miniforge

On {{< meta cluster.name >}}, up-to-date versions of the Miniforge distribution are available via the CBI software stack.  There is no need for you to install this yourself.  To load Miniforge, call:

<!-- code-block label="module-load-miniforge3" -->
```sh
[alice@{{< meta devel.name >}} ~]$ module load CBI miniforge3/24.11.0-0
```
...
```sh
[alice@{{< meta devel.name >}} ~]$ conda --version
conda 24.11.0
[alice@{{< meta devel.name >}} ~]$ python --version
Python 3.12.8
```
...
```sh
[alice@{{< meta devel.name >}} ~]$ conda list
...
```sh
[alice@{{< meta devel.name >}} ~]$ conda create -n myjupyter notebook
Channels:
 - conda-forge
Platform: linux-64
...
## Package Plan ##
  ...
  environment location: {{< meta user.home >}}/.conda/envs/myjupyter
...
```sh
[alice@{{< meta devel.name >}} ~]$ module load CBI miniforge3
[alice@{{< meta devel.name >}} ~]$ conda activate myjupyter
(myjupyter) [alice@{{< meta devel.name >}} ~]$ jupyter notebook --version
7.2.1

(myjupyter) [alice@{{< meta devel.name >}} ~]$ 
```
...
```sh
(myjupyter) [alice@{{< meta devel.name >}} ~]$ conda deactivate
[alice@{{< meta devel.name >}} ~]$ jupyter notebook --version
...
```sh
[alice@{{< meta devel.name >}} ~]$ module load CBI miniforge3
[alice@{{< meta devel.name >}} ~]$ module load CBI conda-stage
[alice@{{< meta devel.name >}} ~]$ conda stage --auto-stage=enable myjupyter
...
Collecting packages...
Packing environment at '{{< meta user.home >}}/.conda/envs/myjupyter' to
'{{< meta user.home >}}/.conda/envs/.tmp.myjupyter.tar.gz'
[########################################] | 100% Completed |  4min  5.6s
INFO: Total 'conda-pack' time: 274 seconds
INFO: Created conda-pack tarball: {{< meta user.home >}}/.conda/envs/myjupyter.tar.gz
...
```sh
[alice@{{< meta devel.name >}} ~]$ conda env export --name myjupyter | grep -v "^prefix: " > myjupyter.yml
[alice@{{< meta devel.name >}} ~]$ ls -l myjupyter.yml
-rw-rw-r-- 1 alice boblab 4982 Aug 21 17:56 myjupyter.yml
```

This configuration file is useful:

* when migrating the environment from {{< meta cluster.nickname >}} to another Conda version, another computer, or another HPC environment
...
# conda environments:
#
myjupyter {{< meta user.home >}}/.conda/envs/myjupyter
...
```sh
[alice@{{< meta devel.name >}} ~]$ CONDA_STAGE=false conda activate myjupyter
(myjupyter) [alice@{{< meta devel.name >}} ~]$ conda list --revisions
...
```sh
(myjupyter) [alice@{{< meta devel.name >}} ~]$ conda install --revision 0
```
...
alice1@{{< meta login.hostname >}}:s password: XXXXXXXXXXXXXXXXXXX
(base) [alice@{{< meta login.name >}} ~]$ 
```
...
```sh
[alice@{{< meta devel.name >}} ~]$ conda info | grep active
conda info | grep active
     active environment : base
    active env location : {{< meta user.home >}}/.conda
```
...
```sh
(base) [alice@{{< meta devel.name >}} ~]$ conda config --set auto_activate_base false
(base) [alice@{{< meta devel.name >}} ~]$ 
```
...
```sh
[alice@{{< meta devel.name >}} ~]$ conda init --reverse
no change     {{< meta user.home >}}/.conda/condabin/conda
no change     {{< meta user.home >}}/.conda/bin/conda
no change     {{< meta user.home >}}/.conda/bin/conda-env
no change     {{< meta user.home >}}/.conda/bin/activate
no change     {{< meta user.home >}}/.conda/bin/deactivate
no change     {{< meta user.home >}}/.conda/etc/profile.d/conda.sh
no change     {{< meta user.home >}}/.conda/etc/fish/conf.d/conda.fish
no change     {{< meta user.home >}}/.conda/shell/condabin/Conda.psm1
no change     {{< meta user.home >}}/.conda/shell/condabin/conda-hook.ps1
no change     {{< meta user.home >}}/.conda/lib/python3.9/site-packages/xontrib/conda.xsh
no change     {{< meta user.home >}}/.conda/etc/profile.d/conda.csh
modified      {{< meta user.home >}}/.bashrc
